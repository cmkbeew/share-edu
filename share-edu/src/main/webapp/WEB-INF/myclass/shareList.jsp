...
<div class="container px-4 py-5">
    ...
    <div id="top" class="mt-4">
        ...
        <div class="d-flex justify-content-between">
            <div class="mb-3" style="text-align: left">
                <button type="button" class="btn btn-outline-secondary" onclick="location.href='/myclass/shareList?member_idx=${sessionScope.member_idx}&type=send'">내가 한 공유</button>
                <button type="button" class="btn btn-outline-secondary" onclick="location.href='/myclass/shareList?member_idx=${sessionScope.member_idx}&type=receive'">내가 받은 공유</button>
            </div>
    ...
    <div class="mb-3">
        <table class="table">
            ...
            <tbody>
            <c:if test="${responseDTO.type == 'send'}">
            ...
            <c:if test="${responseDTO.type == 'receive'}">
                <c:forEach var="list" items="${responseDTO.dtoList}">
                    <tr class="text-center">
                        <td>${list.edu_idx}</td>
                        <td class="text-start"><a href="/myclass/view?edu_idx=${list.edu_idx}" class="text-decoration-none">${list.title}</a></td>
                        <td>${list.share_user_id}</td>
                        <td>${list.reg_date}</td>
                    </tr>
                </c:forEach>
            </c:if>
